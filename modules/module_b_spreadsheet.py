...
def append_leads(leads: list) -> int:
    ...
    with open(PIPELINE_CSV, "a", newline="", encoding="utf-8") as f:
        ...
        for lead in leads:
            ...
            row = {
                "Date Sourced":          today,
                "Week":                  getattr(lead, "week_sourced",       lead.get("week_sourced","")),
                "Confidence Score":      getattr(lead, "confidence_score",   lead.get("confidence_score",0)),
                "Title Tier":            getattr(lead, "title_tier",         lead.get("title_tier","")),
                "Target Name":           getattr(lead, "name",               lead.get("name","")),
                "Company":               getattr(lead, "company",            lead.get("company","")),
                "Position":              getattr(lead, "position",           lead.get("position","")),
                "Target Email":          email,
                "LinkedIn URL":          getattr(lead, "linkedin_url",       lead.get("linkedin_url","")),
                "Location":              getattr(lead, "location",           lead.get("location","")),
                "Geo Segment":           getattr(lead, "geo_segment",        lead.get("geo_segment","")),
                "Vertical":              getattr(lead, "vertical",           lead.get("vertical","")),
                "Company Type":          getattr(lead, "company_type",       lead.get("company_type","")),
                "Funding Stage":         getattr(lead, "funding_stage",      lead.get("funding_stage","")),
                "Is Remote":             str(getattr(lead, "is_remote_role", lead.get("is_remote_role",False))),
                "Outreach Mode":         getattr(lead, "outreach_mode",      lead.get("outreach_mode","")),
                "Company Description":   getattr(lead, "company_description",lead.get("company_description","")),
                "Reason for Outreach":   getattr(lead, "reason_for_outreach",lead.get("reason_for_outreach","")),
                "Drafted Email Subject": "",
                "Drafted Email Body":    "",
                "Status":                "Pending",
                "Sent At":               "",
